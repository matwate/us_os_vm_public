  ...
  public int getPhysicalAddress(int logicalAddress, ProcessMemoryManager pmm, boolean store) {
    ...
    if (pmm.getType() == MemoryManagerType.PAGING) {
      ...
      if (pa == null) {
        ...
        int pageVictim =
            pmmp.getVictim(); // Find a page that needs to leave memory if there is no space
        ...
        if (pageVictim == -1) { // If no victim was found because there are still frames available
          frameVictim =
              getOS().getFreeFrame(); // Obtain a new free frame to store the page from swap
          frameVictimInSwap = -1;
        } else { // If there are no free frames, then a pageVictim was selected
          frameVictim =
              pmmp.getFrameMemoryAddressFromLogicalMemoryAddress(
                  pageVictim); // Find the frame number in memory of the victim page
          frameVictimInSwap =
              pmmp.getVFrameMemoryAddressFromLogicalMemoryAddress(
                  pageVictim); // Find the frame number in Swap memory of the victim page
        }

        int pageToLoad = la.getDivision(); // Get the pageID of the desired page
        int frameToLoadInSwap =
            pmmp.getVFrameMemoryAddressFromLogicalMemoryAddress(
                pageToLoad); // Find the frame number in Swap memory of the desired page

        MemoryPageExchange mpe =
            new MemoryPageExchange(
                pageVictim, frameVictimInSwap, frameVictim, pageToLoad, frameToLoadInSwap);

        if (pageVictim
            != -1) { // If there was a page identified to leave memory, then it may have to be sent
                     // to swap memory
          if (pmmp.isPageDirty(
              pageVictim)) { // If the page is dirty, then it must be updated in the swap memory
            mpe.setFullExchange(
                true); // This is a full exchange, so the frame does not have to be freed
            getOS()
                .interrupt(
                    InterruptType.STORE_PAGE,
                    pmmp.getProcess(),
                    mpe); // Send the frame in memory of page la to swap memory. All the information
                          // needed is in mpe
            pmmp.setPageValid(
                mpe.getFrameVictim(), false); // Set the newly unloaded page as invalid
          }
        }

        getOS()
            .interrupt(
                InterruptType.LOAD_PAGE,
                pmmp.getProcess(),
                mpe); // Load the frame in swap of page la to real memory
        pmmp.setFrameID(pageToLoad, frameVictim);
        pmmp.getPT()
            .getList()
            .get(pageToLoad)
            .setClock(getOS().getClock()); // Set the loading time to the page
